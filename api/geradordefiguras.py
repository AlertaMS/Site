...
def get_raw_file_data(repo_url, pasta='', token=None):
    ...
    for item in contents:
        if item['type'] == 'file' and item['name'].endswith('.csv'):  # Adapte para o formato do seu arquivo
...
def plot_graf_temp(previsoes,cidade, modelos):
    nome_arq = [i for i in os.listdir('./Estacoes/') if i.startswith(cidade) and i.endswith('.txt')][0]
    ...
    fig.write_html(f"./templates/{cidade}.html",config={'modeBarButtonsToRemove': plotly_buttons,'displaylogo': False})
